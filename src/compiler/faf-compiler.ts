/**
 * 🏎️ FAF Scoring Compiler - Mk3.3 (33-Slot Monorepo Edition)
 ...
 *
 * Mk3.3: Adds 12 monorepo slots (21 base + 12 monorepo = 33 total)
 */
...
// ============================================================================
// Design: Base 21 slots for standard projects, 33 slots for monorepos.
// Types define which slot categories COUNT toward scoring.
// slot_ignore in .faf file overrides type defaults for edge cases.
//
// Mk3.3 Update: Added 'monorepo' category with 12 additional slots

/**
 * All slots in the FAF system
 * - Base projects: 21 slots (project + frontend + backend + universal + human)
 * - Monorepos: 33 slots (base 21 + monorepo 12)
 */
const ALL_SLOTS = {
  // Project slots (3)
  ...
  // Monorepo slots (12) - Mk3.3: Added for monorepo projects
  // Infrastructure (5)
  monorepo: [
    ...
    // Application Layers (4) - slotignore for simple monorepos
 ...
 *
 * Mk3.3: Added 'monorepo' category for 33-slot monorepo projects
 */
const TYPE_DEFINITIONS: Record<string, {
  description: string;
  categories: ('project' | 'frontend' | 'backend' | 'universal' | 'human' | 'monorepo')[];
  aliases?: string[];
}> = {
  // ============================================================================
  ...
  // ============================================================================
  // Monorepo Types - Mk3.3: 33-Slot System (21 base + 12 monorepo)
  ...
  //
  // Uses ALL 6 categories: project + frontend + backend + universal + human + monorepo
  // Total slots: 3 + 4 + 5 + 3 + 6 + 12 = 33 slots
  // ============================================================================
  'monorepo': {
    description: 'Monorepo - multi-package repository (33 slots)',
    categories: ['project', 'frontend', 'backend', 'universal', 'human', 'monorepo'],
    aliases: ['mono', 'workspace']
  },
  'turborepo': {
    description: 'Turborepo monorepo (33 slots)',
    categories: ['project', 'frontend', 'backend', 'universal', 'human', 'monorepo'],
    aliases: ['turbo']
  },
  'nx': {
    description: 'Nx monorepo (33 slots)',
    categories: ['project', 'frontend', 'backend', 'universal', 'human', 'monorepo']
  },
  'lerna': {
    description: 'Lerna monorepo (33 slots)',
    categories: ['project', 'frontend', 'backend', 'universal', 'human', 'monorepo']
  },
  'pnpm-workspace': {
    description: 'pnpm workspace monorepo (33 slots)',
    categories: ['project', 'frontend', 'backend', 'universal', 'human', 'monorepo'],
    aliases: ['pnpm-mono']
  },
  'yarn-workspace': {
    description: 'Yarn workspace monorepo (33 slots)',
    categories: ['project', 'frontend', 'backend', 'universal', 'human', 'monorepo'],
    aliases: ['yarn-mono']
  },
...
export class FafCompiler {
  private static readonly VERSION = '3.3.0-compiler-mk3.3';
  private static readonly COMPILER_ID = 'faf-compiler-mk3.3';
  ...
  private buildIR(ast: any): IRSlot[] {
    ...
    let projectType = this.detectProjectTypeFromContext(ast);
    if (process.env.FAF_DEBUG) {
      console.log(`\n[DEBUG] Project type detected: ${projectType}`);
    }

    // 🏢 Enterprise License Check - Monorepo types require valid license
    const monorepoTypes = ['monorepo', 'turborepo', 'nx', 'lerna', 'pnpm-workspace', 'yarn-workspace'];
    if (monorepoTypes.includes(projectType)) {
      // Check for valid enterprise license
      const validator = new LicenseValidator();
      const result = validator.validateSync(); // Synchronous version

      if (!result.valid) {
        // No valid license - fall back to Community Edition (21 slots)
        this.addDiagnostic('warning',
          '⚠️  Monorepo detected but no valid Enterprise license found. ' +
          'Falling back to Community Edition (21 slots). ' +
          'Get a license at https://faf.one/enterprise'
        );

        // Downgrade to library type (9 slots: project + human)
        projectType = 'library';

        if (process.env.FAF_DEBUG) {
          console.log(`[DEBUG] License check failed - downgraded to: ${projectType}`);
        }
      } else {
        // Valid license - proceed with 33-slot scoring
        if (process.env.FAF_DEBUG) {
          console.log(`[DEBUG] ✅ Enterprise license valid - ${result.data?.tier} (${result.data?.customer})`);
        }
      }
    }
    ...
    // Mk3.3: Added monorepo section support
    // Mk3.3.1: Added field aliases for compact format + absent=ignored
    ...
    const getSlotValue = (slotPath: string): any => {
      ...
      if (parts[0] === 'project') {
        ...
        // Mk3.3: Read from monorepo section
        return ast.monorepo?.[parts[1]];
      }
      return undefined;
    };
  ...
  private detectProjectTypeFromContext(ast: any): string {
    ...
    // Monorepo detection (Mk3.3: Check FIRST - monorepos are containers)
    // Detect from monorepo_tool, workspaces, or explicit keywords
    if (ast.stack?.monorepo_tool ||
        ast.stack?.workspaces ||
        ast.monorepo?.packages_count ||
        goal.includes('monorepo') ||
        what.includes('monorepo') ||
        goal.includes('turborepo') ||
        goal.includes('nx workspace') ||
        goal.includes('lerna')) {
      // Determine specific monorepo type if possible
      const tool = (ast.stack?.monorepo_tool || '').toLowerCase();
      if (tool.includes('turbo')) {return 'turborepo';}
      if (tool.includes('nx')) {return 'nx';}
      if (tool.includes('lerna')) {return 'lerna';}
      if (tool.includes('pnpm')) {return 'pnpm-workspace';}
      if (tool.includes('yarn')) {return 'yarn-workspace';}
      return 'monorepo'; // Generic monorepo
    }
